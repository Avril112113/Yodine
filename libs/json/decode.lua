...
local save_sort
local table_pack = table.pack or function(...) local t = {...} t.n = #t return t end
local rawset = rawset
...
local function Utf8(str)
    ...
    elseif n < 0x800 then
        -- [110x xxxx] [10xx xxxx]
        x = string_char(0xC0 + (math.floor(n / 64) % 0x20), 0x80 + (n % 0x40))
    else
        -- [1110 xxxx] [10xx xxxx] [10xx xxxx]
        x = string_char(0xE0 + (math.floor(n / 4096) % 0x10), 0x80 + (math.floor(n / 64) % 0x40), 0x80 + (n % 0x40))
    end
    return x
end
